...
```
PLES  YYMMDDhhmm  ######      e.g. PLES2609081432704218
 4   +    10     +   6    = 20 chars, filling the column exactly
```

Generated by `OrderRepository::nextOrderNumber()`. The tail is `random_int()`,
not a counter: the old `ORD<date><row count>` scheme let anyone holding a receipt
read off how many orders the platform had taken and guess neighbouring numbers.

`order_number` is unique, so `create()` retries up to five times on a collision.
Keep that loop — a collision needs two orders in the same minute drawing the same
six digits, which is rare enough that without a retry it would surface as a
failed checkout nobody could reproduce.

Nothing parses the format; orders are only ever looked up by exact match, so the
older `ORD…` numbers still resolve.
