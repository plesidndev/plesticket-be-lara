...
class CatalogMasterDataService
{
    ...
    {
        $lists = ['genres' => $this->list('genres'), 'languages' => $this->list('languages')];
        $errors = [];
        foreach ($this->fields($metadata) as $path => $type) {
            ...
            $entry = $this->resolve($lists[$type], $value);
            if (! $entry) {
                $errors['metadata.'.$path] = 'Select an active '.($type === 'genres' ? 'genre' : 'language').' from the catalog master data.';
            } else {
    ...
    {
        $lists = ['genres' => $this->list('genres', false), 'languages' => $this->list('languages', false)];
        $result = [];
        foreach ($this->fields($metadata) as $path => $type) {
            $value = data_get($metadata, $path);
            if ($value !== null && ($entry = $this->resolve($lists[$type], $value))) {
    ...
    {
        $fields = ['primary_genre' => 'genres', 'secondary_genre' => 'genres', 'language' => 'languages'];
